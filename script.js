...
const globalGridEl =
  document.getElementById("globalGrid");

const featuredGridEl =
  document.getElementById("featuredGrid");

const exclusiveGridEl =
  document.getElementById("exclusiveGrid");
...
function createProductCard(product) {
  ...
  installment.textContent =
    `10x de ${money(product.price / 10)} sem juros`;
...
function renderBrandFilter() {
  ...
  brands.forEach(brand => {
    ...
    brandFilterEl.appendChild(
      option
    );
  });
}

toggleFiltersEl.addEventListener(
  "click",
  () => {
    filterPanelEl.classList.toggle(
      "active"
    );
  }
);

brandFilterEl.addEventListener(
  "change",
  event => {
    state.brand =
      event.target.value;
...
priceFilterEl.addEventListener(
  "change",
  event => {
    state.price =
      event.target.value;
...
openCartEl.addEventListener(
  "click",
  openCart
);

closeCartEl.addEventListener(
  "click",
  closeCart
);
...
document
  .querySelectorAll(".product-shelf")
  .forEach((shelf) => {
